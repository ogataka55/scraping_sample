...
def search_recipes(base_url, food):
    response = requests.get(f"{base_url}/search/{food}")  # 検索した後のページURLを直接
    html = response.text
    return html
...
def get_recipes(base_url, html):
    ...
    recipe_previews = soup.find_all(class_="recipe-preview")  # find_all 複数

    recipes = []
    for recipe_preview in recipe_previews:
        recipe_title = recipe_preview.find(class_="recipe-title").text  # find 単数
